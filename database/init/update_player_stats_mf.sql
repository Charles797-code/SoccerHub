-- 使用PL/SQL精确分配球员进球数 - 修正版（POSITION = MF）
-- 确保球员进球总和等于俱乐部总进球数
DECLARE
    v_Club_Goals NUMBER;
    v_Club_Id NUMBER;
...
BEGIN
    FOR club_rec IN (
        ...
        SELECT COUNT(CASE WHEN POSITION = 'FW' THEN 1 END),
               COUNT(CASE WHEN POSITION = 'MF' THEN 1 END),
               COUNT(CASE WHEN POSITION = 'DF' THEN 1 END),
               COUNT(CASE WHEN POSITION = 'GK' THEN 1 END)
        INTO v_Fw_Count, v_Mf_Count, v_Df_Count, v_Gk_Count
        FROM PLAYER WHERE CLUB_ID = v_Club_Id AND STATUS = 'ACTIVE';
        
        v_Fw_Goals := ROUND(v_Club_Goals * 0.65);
        v_Mf_Goals := ROUND(v_Club_Goals * 0.25);
        v_Df_Goals := v_Club_Goals - v_Fw_Goals - v_Mf_Goals;
        ...
        FOR fw_rec IN (
            ...
            IF v_Players_Left > 1 THEN
                v_Player_Goals := ROUND((v_Fw_Goals - v_Assigned_Goals) / v_Players_Left);
            ...
            v_Total_Assigned := v_Total_Assigned + v_Player_Goals;
        ...
        v_Assigned_Goals := 0;
        v_Players_Left := v_Mf_Count;
        
        FOR mf_rec IN (
            SELECT PLAYER_ID FROM PLAYER 
            WHERE CLUB_ID = v_Club_Id AND POSITION = 'MF' AND STATUS = 'ACTIVE'
            ORDER BY DBMS_RANDOM.VALUE
        ) LOOP
            IF v_Players_Left > 1 THEN
                v_Player_Goals := ROUND((v_Mf_Goals - v_Assigned_Goals) / v_Players_Left);
                IF v_Player_Goals < 0 THEN v_Player_Goals := 0; END IF;
            ELSE
                v_Player_Goals := v_Mf_Goals - v_Assigned_Goals;
                IF v_Player_Goals < 0 THEN v_Player_Goals := 0; END IF;
            END IF;
            v_Assigned_Goals := v_Assigned_Goals + v_Player_Goals;
            v_Total_Assigned := v_Total_Assigned + v_Player_Goals;
            ...
            SET GOALS = v_Player_Goals, 
                ...
                APPEARANCES = v_Appearances,
                MINUTES_PLAYED = v_Minutes
            WHERE PLAYER_ID = mf_rec.PLAYER_ID;
        END LOOP;
        ...
        FOR df_rec IN (
            ...
            IF v_Players_Left > 1 THEN
                v_Player_Goals := ROUND((v_Df_Goals - v_Assigned_Goals) / v_Players_Left);
                IF v_Player_Goals < 0 THEN v_Player_Goals := 0; END IF;
            ELSE
        ...
        IF v_Total_Assigned != v_Club_Goals THEN
            DECLARE
                v_Diff NUMBER := v_Club_Goals - v_Total_Assigned;
                v_Top_Scorer_Id NUMBER;
            BEGIN
                SELECT PLAYER_ID INTO v_Top_Scorer_Id FROM (
                    SELECT PLAYER_ID FROM PLAYER_SEASON_STATS 
                    WHERE CLUB_ID = v_Club_Id AND GOALS > 0
                    ORDER BY GOALS DESC
                ) WHERE ROWNUM = 1;
                
                UPDATE PLAYER_SEASON_STATS 
                SET GOALS = GOALS + v_Diff
                WHERE PLAYER_ID = v_Top_Scorer_Id;
            END;
        END IF;
    END LOOP;
