...
COMMENT ON TABLE MATCH_EVENT IS '比赛事件表 - 进球/助攻/红黄牌等比赛事件';
COMMENT ON COLUMN MATCH_EVENT.EVENT_TYPE IS '事件类型: GOAL=进球, OWN_GOAL=乌龙球, PENALTY=点球, ASSIST=助攻, YELLOW_CARD=黄牌, RED_CARD=红牌, SUBSTITUTION=换人';
COMMENT ON COLUMN MATCH_EVENT.ASSIST_PLAYER_ID IS '助攻球员ID(仅GOAL/PENALTY类型时有值)';
COMMENT ON COLUMN MATCH_EVENT.MATCH_MINUTE IS '比赛进行分钟数';

-- 3.15 PLAYER_SEASON_STATS (球员赛季统计表)
...
COMMENT ON TABLE PLAYER_SEASON_STATS IS '球员赛季统计表 - 射手榜/助攻榜/红黄牌榜数据来源';
